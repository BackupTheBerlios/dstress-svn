...
public Random new_Random_seed(int seed)
{
    Random R = cast(Random) malloc(Random_struct.sizeof);
...
public Random new_Random(int seed, double left, double right) 
{
    Random R = cast(Random) malloc(Random_struct.sizeof);
...
public void Random_delete(Random R)
{
    free(R);
}
...
public double Random_nextDouble(Random R) 
{
    int k;
    ...
    if (R.haveRange) 
        return  R.left +  dm1 * cast(double) k * R.width;
    else
        return dm1 * cast(double) k;
...
{
    int i;
    double *x = cast(double *) malloc(double.sizeof*N);
...
double **RandomMatrix(int M, int N, Random R)
{
    ...
    for (i=0; i<M; i++)
    {
        A[i] = cast(double *) malloc((double).sizeof*N);
        if (A[i] == null) 
        {
            free(A);
            return null;

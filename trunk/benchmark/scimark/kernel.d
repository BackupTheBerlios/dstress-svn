...
private{
	...
	import scimark.Stopwatch;
	import scimark.SparseCompRow;
	import scimark.array;
}
            ...
            double min_time, Random R)
    {
        ...
        double *y = cast(double*) malloc(double.sizeof*N);
        ...
        int *col = cast(int*) malloc(int.sizeof*nz);
        int *row = cast(int*) malloc(int.sizeof*(N+1));
        int r=0;
        int cycles=1;
    ...
    double kernel_measureLU(int N, double min_time, Random R)
    {
        ...
        if ((pivot = cast(int *) malloc(N * int.sizeof)) == null) exit(1);
